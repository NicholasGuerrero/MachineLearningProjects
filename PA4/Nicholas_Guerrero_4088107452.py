...
from keras.layers import Dense, Dropout, GRU, LSTM, LayerNormalization
...
model.add(Dense(units=512, input_shape=(x_train.shape[1],1)))
model.add(Dropout(0.5))

model.add(LSTM(units=512, return_sequences=True, activation='tanh'))
model.add(Dropout(0.2))

# First GRU layer with Dropout regularisation
model.add(GRU(units=512, return_sequences=True, activation='tanh'))
model.add(Dropout(0.2))
...
# Compiling the RNN
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001, clipnorm=0.8), loss='mean_squared_error')
